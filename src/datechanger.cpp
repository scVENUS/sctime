...
void DateChanger::resetLists()
{
    ...
    if (changeToday && (m_timeMainWindow->abtListToday != m_timeMainWindow->abtList))
    {
        ...
        expectedActions++;
        XMLReader *reader=new XMLReader(m_timeMainWindow->settings, true, false, m_timeMainWindow->abtListToday, m_timeMainWindow->m_punchClockListToday);
    ...
    XMLReader *reader=new XMLReader(m_timeMainWindow->settings, true, false, m_timeMainWindow->abtList, m_timeMainWindow->m_punchClockList);
    connect(reader, &XMLReader::settingsRead, this, &DateChanger::updatePunchClock);
    connect(reader, &XMLReader::settingsRead, reader, &XMLWriter::deleteLater);
    connect(reader, &XMLReader::offlineSwitched, [this](bool offline){emit offlineSwitched(offline);});
    reader->open();
